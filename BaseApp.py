...
class BaseApp(YandexPages):
    ...
    def __init__(self, app):
        super().__init__()
        self.browser = app
        # self.browser = webdriver.Remote(command_executor="http:
        # //192.168.0.101:4444/wd/hub",desired_capabilities={"browserName":
        # "firefox",})
        # в качестве примера испольузется обычный браузер , а ниже сам selenium
        # grid (настроенный)
    ...
    def find_element(self, locator: str, time: int = 10) -> str:
        return WebDriverWait(self.browser, time).until(EC.visibility_of_element_located(locator), message=f"Can't find eleent by locator {locator}")
        # чтобы часто не использовать этот метод в YandexPages,
        # я просто буду ссылаться на него
